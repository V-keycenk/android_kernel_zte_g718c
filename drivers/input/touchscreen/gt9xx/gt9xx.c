...
static u8 proc_fw_infomation[8]; 
static u8 gtp_is_gesture = 0;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	gtp_is_gesture = input;

	return count;
}

static int proc_read_val(char *page, char **start, off_t off, int count, int *eof,
	  void *data)
{
	int len = 0;
	...
	len += sprintf(page + len, "firmware version : %c%c%c_%02x%02x\n", proc_fw_infomation[3],proc_fw_infomation[4],proc_fw_infomation[5],proc_fw_infomation[6],
	proc_fw_infomation[7]);

    switch (proc_fw_infomation[2]){
	case 0x0:
        ...
        {      
            ...
            {   
                ...
                else if (0xCC == doze_buf[2])
                {
    				...
    				{
                        doze_status = DOZE_WAKEUP;
                        input_report_key(ts->input_dev, KEY_POWER_ON, 1);
                        input_sync(ts->input_dev);
                        input_report_key(ts->input_dev, KEY_POWER_ON, 0);
                        input_sync(ts->input_dev);                        
*/
    ...
    {
        GTP_INFO("gtp is suspend,return from %s",__func__);
        return;
    }
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    proc_fw_infomation[2] = sensor_id;//added by zte
...
s32 gtp_read_version(struct i2c_client *client, u16* version)
{
    ...
    if (buf[5] == 0x00)
    {
		...
		proc_fw_infomation[3] = buf[2];//added by zte
		proc_fw_infomation[4] = buf[3];
		proc_fw_infomation[5] = buf[4];
		proc_fw_infomation[6] = buf[7];
		proc_fw_infomation[7] = buf[6];
    }
    else
    {
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    input_set_capability(ts->input_dev, EV_KEY, KEY_POWER_ON);
    input_set_capability(ts->input_dev, EV_KEY, KEY_UNLOCK);
    input_set_capability(ts->input_dev, EV_KEY, KEY_CAMERA);
    input_set_capability(ts->input_dev, EV_KEY, KEY_PREVIOUSSONG);
    input_set_capability(ts->input_dev, EV_KEY, KEY_NEXTSONG);
    input_set_capability(ts->input_dev, EV_KEY, KEY_PHONE);
    input_set_capability(ts->input_dev, EV_KEY, KEY_WWW);
    input_set_capability(ts->input_dev, EV_KEY, KEY_MESSAGE);
    input_set_capability(ts->input_dev, EV_KEY, KEY_UNDEFINE_S);
    input_set_capability(ts->input_dev, EV_KEY, KEY_UNDEFINE_W);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    {
        GTP_INFO("Have another tp register");
        tpd_load_status=0;
        return ret;
    }
    ...
    {
        GTP_INFO("Read version is %d",version_info);
    }
    ...
    ret = sysfs_create_file(&ts->input_dev->dev.kobj,
    		&attrs[0].attr);
    if (ret < 0) {
        printk("%s:create attr file failed\n",__func__);
    }
...
{  
    ...
    if(ts->gtp_is_suspend==1){
        GTP_INFO("gtp_is_suspend has been 1, return");        
        return;
    }
...
static void goodix_ts_resume(struct goodix_ts_data *ts)
{
    ...
    if(ts->gtp_is_suspend==0){
        GTP_INFO("gtp_is_suspend has been 0, return");        
        return;
    }
    ...
    GTP_INFO("%s,set gtp_is_suspend to 0",__func__);
    ts->gtp_is_suspend = 0;
#if GTP_ESD_PROTECT
    gtp_esd_switch(ts->client, SWITCH_ON);
#endif
...
static int __devinit goodix_ts_init(void)
{
    s32 ret;
   //added by zte, begin
   ret = boot_is_poweroff_charger();
   if(ret)
   	{
   		printk("GTP driver not install when poweroff charger ...\r\n");
   		return 0;
   	}
   	else
   	 { 		
   	 	printk("install the gt dirver\r\n");
   	 }
   //added by zte, end
