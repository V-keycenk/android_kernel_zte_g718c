...
#define GESTURE_DETECT_DELAY_MS  20 
...
#define F11_FAST_RELAXATION_RATE_ON 0x0A
#define F11_FAST_RELAXATION_RATE_OFF 0xC8
#define F12_CONTINUOUS_MODE 0x00
#define F12_WAKEUP_GESTURE_MODE 0x02

static struct wakeup_source *gesture_ws;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if (retval < 0){
            printk("%s, detecte gestures failed\n",__func__);
			return 0;
		}
		...
		if (detected_gestures) {
            ...
            printk("%s:detected_gestures is %d\n",__func__,detected_gestures);

            if (detected_gestures==1){
                printk("%s,KEY_POWER_ON\n",__func__);
                code = KEY_POWER_ON;
            } else if (detected_gestures==2){
                ...
                switch(direction){
                    ...
                    case 0x1:
                        ...
                        break;
                    default:
                        printk("%s:not know this direction\n",__func__);
                }
            }else if (detected_gestures==64){
               	unsigned char charater[8];
        		retval = synaptics_rmi4_reg_read(rmi4_data,
        		data_addr + extra_data->data39_offset,
        		charater,8);
                
           		if (retval < 0)
        			return 0;
                
                printk("%s,charater is 0x%x\n",__func__,charater[6]);
                switch(charater[6]){
                    case 0x63:
                        printk("%s,KEY_PHONE\n",__func__);
                        code = KEY_PHONE;
                    ...
                    case 0x65:
                        ...
                        break;
                    case 0x6d:
                        printk("%s,KEY_MESSAGE\n",__func__);
                        code = KEY_MESSAGE;
                        break;
                    case 0x73:
                        printk("%s,KEY_UNDEFINE_S\n",__func__);
                        code = KEY_UNDEFINE_S;
                        break;
                    case 0x77:
                        printk("%s,KEY_UNDEFINE_W\n",__func__);
                        code = KEY_UNDEFINE_W;
                        break;
                    default:
                        printk("%s:not know this charater\n",__func__);
                }
            }else
                printk("%s:not support this gestures\n",__func__);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if (detected_gestures) {
			input_report_key(rmi4_data->input_dev, KEY_POWER_ON, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev, KEY_POWER_ON, 0);
			input_sync(rmi4_data->input_dev);
		...
		bool enable)
{
    ...
    printk("%s:intr_mask is %0x\n",__func__,intr_mask[0]);
  	intr_mask[0] = 0x16;
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
        printk("%s:receive a gesture\n",__func__);
        __pm_stay_awake(gesture_ws);
        synaptics_rmi4_int_enable(rmi4_data,false);	
        queue_delayed_work(rmi4_data->gesture_workqueue,
			&rmi4_data->gesture_work,
			msecs_to_jiffies(GESTURE_DETECT_DELAY_MS));
	} else {
		...
		unsigned int intr_count)
{
	...
	if (query_0_5.has_query_27 && query_27.has_wakeup_gesture){
		extra_data->data38_offset = offset;
   		extra_data->data39_offset = offset+1;
	}
...
static int synaptics_rmi4_query_device(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    printk("into synaptics_rmi4_query_device\n");
	rmi = &(rmi4_data->rmi4_mod_info);
...
rescan_pdt:
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			if (retval < 0) {
                dev_err(rmi4_data->pdev->dev.parent,
						"%s: synaptics_rmi4_reg_read return less 0_1\n",
						__func__);
				return retval;
			}
			...
			case SYNAPTICS_RMI4_F01:
				...
				if (retval < 0){
                dev_err(rmi4_data->pdev->dev.parent,
						"%s: synaptics_rmi4_f01_init return less 0\n",
						__func__);
				return retval;
			}
			...
			case SYNAPTICS_RMI4_F11:
				...
				if (retval < 0){
                dev_err(rmi4_data->pdev->dev.parent,
						"%s: synaptics_rmi4_f11_init return less 0\n",
						__func__);
				return retval;
			    }
				break;
			case SYNAPTICS_RMI4_F12:
				...
				if (retval < 0){
                    dev_err(rmi4_data->pdev->dev.parent,
    						"%s: synaptics_rmi4_f12_init return less 0\n",
    						__func__);
    				return retval;
			    }
				break;
...
#else
                    dev_err(rmi4_data->pdev->dev.parent,
    						"%s: synaptics_rmi4_f1a_init return less 0\n",__func__);
					return retval;
#endif
				}
				break;
...
flash_prog_mode:
	...
	if (retval < 0){
        dev_err(rmi4_data->pdev->dev.parent,
				"%s: synaptics_rmi4_reg_read return  less 0_2\n",
				__func__);
		return retval;
    }
	...
	if (retval < 0){
        dev_err(rmi4_data->pdev->dev.parent,
				"%s: synaptics_rmi4_reg_read return less 0_3\n",
				__func__);
		return retval;
    }
    ...
    printk("leave synaptics_rmi4_query_device\n"); 
	return 0;
}
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
		set_bit(KEY_POWER_ON, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_POWER_ON);      
   		set_bit(KEY_UNLOCK, rmi4_data->input_dev->keybit);
        input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_UNLOCK);        
   		set_bit(KEY_CAMERA, rmi4_data->input_dev->keybit);
        input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_CAMERA);
   		set_bit(KEY_PREVIOUSSONG, rmi4_data->input_dev->keybit);
        input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_PREVIOUSSONG);
   		set_bit(KEY_NEXTSONG, rmi4_data->input_dev->keybit);
        input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_NEXTSONG);
   		set_bit(KEY_PHONE, rmi4_data->input_dev->keybit);
        input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_PHONE);
   		set_bit(KEY_WWW, rmi4_data->input_dev->keybit);
        input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_WWW);
   		set_bit(KEY_MESSAGE, rmi4_data->input_dev->keybit);
        input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_MESSAGE);
   		set_bit(KEY_UNDEFINE_S, rmi4_data->input_dev->keybit);
        input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_UNDEFINE_S);
   		set_bit(KEY_UNDEFINE_W, rmi4_data->input_dev->keybit);
        input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_UNDEFINE_W);
	}
...
static void gesture_detect_work(struct work_struct *work)
{
   	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;

    printk("%s:synaptics_rmi4 enable irq source\n",__func__); 
    synaptics_rmi4_int_enable(rmi4_data,true);
    
	
    synaptics_rmi4_sensor_report(rmi4_data);
    
   	__pm_relax(gesture_ws);
	return;
}
...
static int mt_tpd_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
   int len = 0;
   char *p = buf;
   struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data; 
   ...
   printk("=== Syna [mt_tpd_read]%s ,line=%d,firmware_information[0]=0x%x, 0x%x, 0x%x, 0x%x \n", __func__,__LINE__,firmware_information[0],firmware_information[1],firmware_information[2],firmware_information[3]);
...
static int __devinit synaptics_rmi4_probe(struct platform_device *pdev)
{
    ...
    tpd_load_status = 1; 
    ...
    #ifdef CONFIG_FB
        printk("wangchong register suspend1\r\n");
    	...
    	printk("wangminrong register suspend\r\n");
    #endif
	...
	mt_entry = create_proc_entry("driver/tsc_id", 0644, NULL);
	if (mt_entry)
	{
	    mt_entry->read_proc = mt_tpd_read;
	    mt_entry->write_proc = NULL;
	}

    gesture_ws = wakeup_source_register("autosleep");
	if (!gesture_ws)
        printk("%s:get gesture_ws failed\n",__func__);
     
    rmi4_data->gesture_workqueue = create_singlethread_workqueue("gesture_workqueue");
    INIT_DELAYED_WORK(&rmi4_data->gesture_work, gesture_detect_work);
...
static int __devexit synaptics_rmi4_remove(struct platform_device *pdev)
{
    ...
    cancel_delayed_work_sync(&rmi4_data->gesture_work);
    flush_workqueue(rmi4_data->gesture_workqueue);
    destroy_workqueue(rmi4_data->gesture_workqueue);
		...
		bool enable)
{
	...
	if (enable){
        ...
        fast_relaxation_rate = F11_FAST_RELAXATION_RATE_ON;
	}
	else {
		reporting_control |= F11_CONTINUOUS_MODE;
        fast_relaxation_rate = F11_FAST_RELAXATION_RATE_OFF;
	}
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			0x011D,
			&fast_relaxation_rate,
			sizeof(fast_relaxation_rate));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to change fast relaxation rate\n",
				__func__);
		return;
	}
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if (rmi4_data->enable_wakeup_gesture) {
		...
		synaptics_rmi4_irq_enable(rmi4_data, false, false);
        printk("%s:synaptics_rmi4_irq_enable\n",__func__);
