...
#define FW_IMAGE_NAME_ECW "dsx_ecw_3041.img"
#define FW_IMAGE_NAME_TPK "dsx_tpk_3039.img"
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source) {
    	...
    	switch (fwu->productinfo1){
       	case 0x1f:
            printk("%s:tpk\n",__func__);
    		firmware_name = FW_IMAGE_NAME_TPK;
    		break;
    	case 0x27:
            printk("%s:ecw\n",__func__);
    		firmware_name = FW_IMAGE_NAME_ECW;
    		break;
    	 default:
            printk("%s:not support 0x%04x product\n",__func__,fwu->productinfo1);
  			retval = -EINVAL;
			goto exit;
    	}
            
		strncpy(fwu->image_name, firmware_name, MAX_IMAGE_NAME_LEN);
		dev_dbg(rmi4_data->pdev->dev.parent,
